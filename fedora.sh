...
controllerImgs=(flannel hyperkube kubedns-amd64 kube-dnsmasq-amd64 dnsmasq-metrics-amd64 exechealthz-amd64 cluster-proportional-autoscaler-amd64 heapster addon-resizer kubernetes-dashboard-amd64 node cni kube-policy-controller)

workerImgs=(flannel hyperkube gb-frontend redis gb-redisslave)
...
while read line; do
 ...
 [[ "${controllerImgs[@]}" =~ " $imageBase " ]] && echo "Load $imageBase...." || continue
